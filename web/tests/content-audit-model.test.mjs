...
const rawSituations = [...catalogA.situations, ...catalogB.situations];
...
test("la auditoría clasifica los cien casos una sola vez", () => {
  ...
  assert.equal(new Set(audit.map((item) => item.id)).size, 100);
  assert.equal(Object.values(summary.counts).reduce((sum, value) => sum + value, 0), 100);
