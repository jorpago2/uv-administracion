...
function bindCalculator(root, selector, render) {
  ...
  const update = () => {
    try {
      ...
      const status = form.closest(".decision-card").querySelector(".decision-result__status");
      status.textContent = error instanceof Error ? error.message : "No se pudo realizar el cálculo.";
    }
  };
...
function renderPurchase(form) {
  ...
  setText(panel, ".decision-result__status", result.status === "minor" ? "No realices el pedido hasta completar la adjudicación y el AD." : "Ruta orientativa según IUV 1/2025.");
  renderBreakdown(panel, [
    ["Importe sin IVA", money.format(result.amount)],
    ["Ofertas mínimas", result.offers ? String(result.offers) : "No aplica"],
    ["AD previo", result.requiresAd ? "Sí" : "No, salvo excepción"],
    ["Límite menor", result.minorLimit === null ? "Umbral SARA vigente" : money.format(result.minorLimit)]
  ]);
...
function renderTravel(form) {
  const result = calculateTravel({
    ...
    lodgingDays: form.elements.lodgingDays.value,
    fullMealDays: form.elements.fullMealDays.value,
    halfMealDays: form.elements.halfMealDays.value,
  ...
  setText(panel, ".decision-result__status", `Máximo planificado · ${result.destinationLabel}. Alojamiento ${money.format(result.lodgingRate)}/noche y manutención ${money.format(result.mealRate)}/día.`);
  renderBreakdown(panel, [
...
const EXAMPLES = Object.freeze({
  ...
  "viaje-congreso-eindhoven": { form: "#travelForm", values: { funding: "uv", destinationType: "foreign", foreignId: travelData.foreign.find((item) => item.label === "Països Baixos")?.id || "", lodgingDays: 3, fullMealDays: 2, halfMealDays: 2, actualLodging: 510, publicTransport: 185, registration: 420, kilometres: 0, vehicle: "car", distance: 1800, exceedsWorkday: true } },
  "contrato-investigador-doce-meses": { form: "#personnelForm", values: { grossAnnual: 32000, months: 12, contractType: "fixed", accidentRate: 1.5, otherRate: 0, otherCosts: 0, reserveRate: 2 } }
});
